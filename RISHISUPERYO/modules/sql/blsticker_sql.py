...
class StickersFilters(BASE):
    tablename = "blacklist_stickers"
    chat_id = Column(String(14), primary_key=True)
    trigger = Column(UnicodeText, primary_key=True, nullable=False)

    def init(self, chat_id, trigger):
...
class StickerSettings(BASE):
    tablename = "blsticker_settings"
    chat_id = Column(String(14), primary_key=True)
    blacklist_type = Column(Integer, default=1)
    value = Column(UnicodeText, default="0")

    def init(self, chat_id, blacklist_type=1, value="0"):
...
def load_chat_stickerset_blacklists():
    global CHAT_BLSTICK_BLACKLISTS
    try:
...
load_CHAT_STICKERS()
load_chat_stickerset_blacklists()
